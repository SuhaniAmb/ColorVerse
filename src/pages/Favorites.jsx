...
function Favorites() {
  ...
  const favoriteColors = [
    ...new Map(
      categories
        ...
        .filter((color) => favorites.includes(color.code))
  ...
  return (
    <div className=" bg-slate-50 dark:bg-gray-900 px-6 md:px-10 ">
      <h1 className="text-3xl text-gray-900 dark:text-white font-bold mb-5">
        Favorites Colors
      </h1>

      {favoriteColors.length === 0 ? (
        <p className="text-gray-500 dark:text-white text-center my-40">
          No favorite colors yet ❤️
        </p>
      ) : (
          ...
          {favoriteColors.map((item, index) => (
            <div key={item.code}>
              <div
                ...
                onClick={() => copyColor(item.code, index)}
                className="lg:h-36 h-30 shadow-md rounded-lg cursor-pointer hover:scale-95 active:scale-90 transition duration-300 relative group "
              >
